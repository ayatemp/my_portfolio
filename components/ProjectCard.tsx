    ...
    >
      <Link href={`/${locale}/projects/${project.slug}`} className="block h-full">
        <div className="relative h-80 border border-white/[0.08] rounded-xl overflow-hidden bg-bg-surface group">
          ...
          {/* Card number */}
          <div className="absolute top-6 right-6 flex items-center gap-2 font-mono text-[10px] text-white/10 group-hover:text-accent-amber/30 transition-colors">
            {isSpotlight ? (
              <span aria-hidden="true" className="text-sm leading-none text-accent-amber/75">
                ★
              </span>
            ) : null}
